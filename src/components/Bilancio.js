...
export const Bilancio = () => {
  ...
  const totale = bilancio.reduce((acc, item) => (acc + item), 0)
  
  return (
    <div className="row m-auto rounded-4 card-pers shadow text-truncate">
      <div className="col mt-5 mb-3 text-center">
        ...
        <span id="bilancio" className='ms-1 bilancio text-light'>{currencyFormatter(totale)}</span>
      </div>
